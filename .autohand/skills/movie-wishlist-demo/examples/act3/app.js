// Movie Wishlist — TMDb search app with localStorage-persisted wishlist (Act III)
// Loaded as a classic (non-module) script so the app works from a file:// URL.
// createCrud is provided by storage.js (loaded first) on the global scope.

const API_BASE = "https://api.themoviedb.org/3";
const POSTER_BASE = "https://image.tmdb.org/t/p/w342";

const wishlistStore = createCrud("movie-wishlist", []);
...
function loadWishlist() {
  const entries = wishlistStore.get();
  if (!Array.isArray(entries)) return;
  entries.forEach(function (entry) {
    if (entry && entry.movie && typeof entry.movie.id === "number") {
      // Key by string id to match toggleWishlist/setRating/remove lookups.
      wishlist.set(String(entry.movie.id), {
        movie: entry.movie,
...
function saveWishlist() {
  ...
  wishlist.forEach(function (entry) {
    entries.push({ movie: entry.movie, rating: entry.rating });
  });
  wishlistStore.set(entries);
}
